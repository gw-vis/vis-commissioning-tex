...
One of the typical choices of $J$ would be the (weighted) $\mathcal{L}_2$ norm, which is equivalent to expected RMS of the super sensor noise, i.e.
\begin{equation}
	J_\mathrm{RMS}\mleft( H_1(s) \mright) = \left[\int_{-\infty}^\infty \, \hat{N}_\mathrm{super}\mleft(f;H_1(s)\mright)^2 w(f)^2\, df\right]^\frac{1}{2}\,,
\end{equation}
where, again, $w(f)$ is some specified weighting function according to specific requirements of the sensor.
Another typical choice of $J$ would be the (weighted) $\mathcal{L}_\infty$ norm, which corresponds to setting frequency dependent specifications of the super sensor noise:
\begin{equation}
	J_{\mathcal{L}_\infty} = \sup_{f} \hat{N}_\mathrm{super}\mleft(f;H_1(s)\mright)
\end{equation}
